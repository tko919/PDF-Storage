...
\begin{align*}
    I(U,V):=\int_0^a (\langle \nabla_{\dot{\gamma}} U, \nabla_{\dot{\gamma}} V \rangle-\langle V,R(U,\dot{\gamma})\dot{\gamma} \rangle) ds
\end{align*}
...
\prf 2点 $p,q \in M$ をとり $\rho(p,q)=l<D$ とする。定理\ref{thmhopf}より最短測地線 $\gamma:[0,l] \to M$ が存在する。$\{e_i\}_{i=1}^n \ (e_1=\dot{\gamma})$ を $\gamma$ に沿った平行なベクトル場で、各点の接空間の正規直交基底を成すとする。そして $U_i=\sin(\pi s/l)e_i$ とおくと、 $U_i$ は全域で滑らかなことに注意して
\begin{align*}
    I(U_i,U_i) &=\int_0^l (|\nabla_{\dot{\gamma}}U_i|^2-\ev{R(U_i,\dot{\gamma})\dot{\gamma},U_i}) ds\\
    &=-\int_0^l \ev{U_i,\nabla_{\dot{\gamma}}\nabla_{\dot{\gamma}}U_i+R(U_i,\dot{\gamma})\dot{\gamma}} ds \\
    &=\int_0^l \sin^2(\pi s/l)(\pi^2/l^2-\ev{R(e_i,\dot{\gamma})\dot{\gamma},e_i}) ds \\ % ??
    \sum_{i=2}^n I(U_i,U_i) &=\int_0^l \sin^2(\pi s/l)((n-1)\pi^2/l^2-\mbox{Rc}(\dot{\gamma},\dot{\gamma})) ds
\end{align*}

ここで $l>\pi/\sqrt{\lambda}$ とすると右辺は負になるので、$\exists U_i,I(U_i,U_i)<0$ 。よって $I$ は半正定値でないので $\gamma$ の最短性に矛盾する。 \qed
